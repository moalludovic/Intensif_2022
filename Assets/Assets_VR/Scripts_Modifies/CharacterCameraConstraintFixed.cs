...
public class CharacterCameraConstraintFixed : MonoBehaviour
{
    // MODIFICATION SCRIPT ORIGINAL : POUR SCREENFADE DANS LES COLLISIONS
    ...
    CharacterCameraConstraintFixed()
    {
    ...
    void PreCharacterMovement()
    {
        ...
        // MODIFICATION SCRIPT ORIGINAL : POUR SCREENFADE DANS LES COLLISIONS
        if (EnableFadeout && CurrentDistance >= FadeMinDistance)
        {
            float fadeLevel = Mathf.Clamp01((CurrentDistance - FadeMinDistance) / (FadeMaxDistance - FadeMinDistance));
            fader.SetExplicitFade(fadeLevel * MaxFade);
        }
        else if (EnableFadeout && CurrentDistance <= FadeMinDistance)
        {
